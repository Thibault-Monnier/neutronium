...
/** A vector-like container that stores a small number of elements inline, and allocates memory on
 * the heap when the number of elements exceeds the inline capacity.
 *
 * @tparam T The type of the elements stored in the vector. Must have an alignment less than or
 * equal to `std::max_align_t`.
 * @tparam DEFAULT_CAPACITY The number of elements to store inline before allocating memory on the
 * heap. If left unspecified, defaults to a number of elements that fit within a small inline
 * buffer, e.g., 128 bytes.
 */
